/**
 * Importa leads Meta (Taller Profesional Global — 30 jul 2026) a GoHighLevel.
 *
 ...
 * - Tags: taller-profesional-global-2026 (+ meta-julio30 para trazabilidad)
 * - Crea oportunidad en pipeline taller si no hay una abierta del contacto.
 *
 * Uso (desde more-landing/):
 *   node scripts/import-ghl-meta-julio30.mjs <ruta_xls> [--dry-run] [--limit=N]
 */
...
if (!filePathArg) {
  console.error("Uso: node scripts/import-ghl-meta-julio30.mjs <ruta_xls> [--dry-run]")
  process.exit(1)
}
...
const PRIMARY_TAG = "taller-profesional-global-2026"
const SOURCE_TAG = "meta-julio30"
const TAGS = [PRIMARY_TAG, SOURCE_TAG]
const SOURCE = "Meta Lead Ads — Taller Profesional Global Julio 30 2026"
const EVENT_LABEL = "Taller Profesional Global"
...
async function main() {
  ...
  const reportPath = path.join(outDir, `import-meta-julio30-${ts}.json`)
