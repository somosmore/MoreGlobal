...
const KPI_CONFIG = [
  { key: "total",       label: "Total leads",  icon: Users,        color: "text-[#2A3A4A]",  bg: "bg-[#2A3A4A]/5" },
  { key: "altoImpacto", label: "Alto Impacto",  icon: Sparkles,     color: "text-[#F37021]",  bg: "bg-[#F37021]/8" },
      ...
      {KPI_CONFIG.map(({ key, label, icon: Icon, color, bg }) => (
        <Card key={key} className="border-0 shadow-sm">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-[#2A3A4A] leading-none">{stats[key]}</p>
