...
/** Identificador de la landing para distinguir estos leads en la base de datos / GHL. */
const COMUNIDAD_SOURCE = "comunidad-facebook"
const COMUNIDAD_EVENT_LABEL = "Comunidad MORE — Grupo de Facebook"
/** Etiqueta (tag) con la que se marcan los leads en GoHighLevel. */
const COMUNIDAD_GHL_TAG = "Comunidad-Facebook"
...
function validate(data: FormData): FormErrors {
  ...
  // País se deriva de la lada → no se valida.
  return errors
}
...
type CMRegistrationFormProps = {
  variant?: ComunidadFormVariant
}

export default function CMRegistrationForm({ variant = "default" }: CMRegistrationFormProps) {
    ...
    try {
      ...
      const res = await fetch(edgeFunctionUrl, {
        ...
        body: JSON.stringify({
          ...
          phone: fullPhone,
          pais,
          source: COMUNIDAD_SOURCE,
          event_label: COMUNIDAD_EVENT_LABEL,
          ghl_tag: COMUNIDAD_GHL_TAG,
          ...utmParams,
  ...
  return (
    ...
    >
      <div className="relative max-w-lg mx-auto px-4 sm:px-6">
        ...
        <AnimatePresence mode="wait">
          {submitted ? (
            <CMFacebookJoinCard key="success" variant={variant} />
          ) : (
            <motion.form
              key="form"
