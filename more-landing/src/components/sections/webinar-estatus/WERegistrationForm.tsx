...
import {
  getWebinarEstatusFormCopy,
  type WebinarEstatusFormVariant,
} from "@/components/sections/webinar-estatus/webinarEstatusCopy"
import "flag-icons/css/flag-icons.min.css"

import WEWhatsappJoinCard from "@/components/sections/webinar-estatus/WEWhatsappJoinCard"
/** Identificador del evento para distinguir estos leads en la base de datos / GHL. */
const WEBINAR_SOURCE = "webinar-estatus-2026"
const WEBINAR_EVENT_LABEL = "Webinar Estatus de Verdad"
/** Etiqueta (tag) con la que se marcan los leads en GoHighLevel. */
const WEBINAR_GHL_TAG = "Webinar-estatus-2026"
...
function SuccessCard({ variant }: { variant: WebinarEstatusFormVariant }) {
  return <WEWhatsappJoinCard variant={variant} />
}
...
type WERegistrationFormProps = {
  variant?: WebinarEstatusFormVariant
}

export default function WERegistrationForm({ variant = "default" }: WERegistrationFormProps) {
  const copy = getWebinarEstatusFormCopy(variant)
    ...
    try {
      ...
      const res = await fetch(edgeFunctionUrl, {
        ...
        body: JSON.stringify({
          ...
          event_label: WEBINAR_EVENT_LABEL,
          ghl_tag: WEBINAR_GHL_TAG,
          ...utmParams,
  ...
  const { expired: isExpired } = useOfferWindow(settings.we_registration_closes_at)
