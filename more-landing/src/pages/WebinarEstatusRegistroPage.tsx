...
import { WEBINAR_ESTATUS_REGISTRO_PAGE_META } from "@/components/sections/webinar-estatus/webinarEstatusCopy"
import { Backdrop } from "@/components/brand/Backdrop"
import { CtaButton } from "@/components/brand/CtaButton"

export default function WebinarEstatusRegistroPage() {
  const { status, isAccessible, reason } = useLandingStatus("/webinar-estatus")

  useEffect(() => {
    document.title = WEBINAR_ESTATUS_REGISTRO_PAGE_META.title
    ...
    setMeta("name", "description", WEBINAR_ESTATUS_REGISTRO_PAGE_META.description)
    setMeta("property", "og:title", WEBINAR_ESTATUS_REGISTRO_PAGE_META.ogTitle)
    setMeta(
      "property",
      "og:description",
      WEBINAR_ESTATUS_REGISTRO_PAGE_META.ogDescription
    )
  ...
  return (
    <div className="campaign-editorial relative flex min-h-[100dvh] flex-col overflow-hidden bg-paper">
      ...
      <main className="relative z-10 flex flex-1 flex-col justify-center px-2 py-10 sm:py-14">
        <WEWhatsappJoinCard variant="ads" showLogo />
        <p className="relative z-10 mt-8 text-center font-sans text-xs text-ink-muted">
          <Link
            to="/"
            className="rounded-sm underline-offset-4 transition-colors hover:text-navy hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange"
          >
            Volver al inicio
          </Link>
        </p>
