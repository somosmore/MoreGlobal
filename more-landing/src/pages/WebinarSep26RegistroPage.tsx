...
import WSWhatsappJoinCard from "@/components/sections/webinar-sep-26/WSWhatsappJoinCard"
import { WEBINAR_SEP26_REGISTRO_PAGE_META } from "@/components/sections/webinar-sep-26/webinarSep26Copy"
import { Backdrop } from "@/components/brand/Backdrop"
import { CtaButton } from "@/components/brand/CtaButton"

export default function WebinarSep26RegistroPage() {
  const { status, isAccessible, reason } = useLandingStatus("/webinar-sep-26")

  useEffect(() => {
    document.title = WEBINAR_SEP26_REGISTRO_PAGE_META.title
    ...
    setMeta("name", "description", WEBINAR_SEP26_REGISTRO_PAGE_META.description)
    setMeta("property", "og:title", WEBINAR_SEP26_REGISTRO_PAGE_META.ogTitle)
    setMeta(
      "property",
      "og:description",
      WEBINAR_SEP26_REGISTRO_PAGE_META.ogDescription
    )
  ...
  return (
    <div className="campaign-editorial relative flex min-h-[100dvh] flex-col overflow-hidden bg-paper">
      ...
      <main className="relative z-10 flex flex-1 flex-col justify-center px-2 py-10 sm:py-14">
        <WSWhatsappJoinCard variant="ads" showLogo />
